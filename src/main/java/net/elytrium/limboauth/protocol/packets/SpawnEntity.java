...
package net.elytrium.limboauth.protocol.packets;
...
public class SpawnEntity implements MinecraftPacket {
  ...
  public SpawnEntity(int id, UUID uuid, Function<ProtocolVersion, Integer> type, double positionX, double positionY,
      double positionZ, float pitch, float yaw, float headYaw, int data) {
    this.id = id;
    this.uuid = uuid;
    this.type = type;
  ...
  public void encode(ByteBuf buf, ProtocolUtils.Direction direction, ProtocolVersion protocolVersion) {
    ...
    if (this.data != 0 || protocolVersion.compareTo(ProtocolVersion.MINECRAFT_1_9) >= 0) {
      buf.writeShort(0);
      buf.writeShort(0);
      buf.writeShort(0);
    }
